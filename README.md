...
- **外挂歌词**：与音频同名的 `.lrc` / `.qrc` 文件（`.qrc` 逐字歌词在播放页按字高亮，加密格式在 Rust 侧自动解密；`.lrc` 支持**增强版 LRC** 的行内 `<mm:ss.xx>` 字级时间戳，同样逐字高亮）；或标签内嵌歌词（ID3v2 USLT / Vorbis LYRICS / M4A）
