...
- **听歌统计**：侧栏「听歌统计」页（入口**默认隐藏**，设置 → 通用可开启），分两个标签：
  - **收听统计**——汇总卡（累计 / 今日 / 近 7 天收听时长、总播放次数、有效播放次数（一曲一天一次）、独立曲目 / 艺人数、连续听歌天数（当前与最长））；收听趋势柱状图（按日 / 周 / 月切换）；**星期 × 小时热力图**（看一周内什么时段听得最多）；「听得最多的歌」榜单（曲目 / 艺人 / 专辑 / 流派 四类 × 周 / 月 / 全部）；播放模式占比（顺序 / 循环 / 单曲 / 随机）与播放来源占比（本地 / WebDAV）
  - **音乐库体检**——曲库概览（曲目 / 专辑 / 艺人 / 流派数、总时长、总大小）；覆盖率（歌词（内嵌 / 外挂 / 逐字细分）、封面、MV 关联、元数据完整率六字段）；分布（格式、来源、码率、采样率、位深、年份）；疑似重复曲目组；最近扫描历史（增 / 改 / 删 / 耗时）
